...
1. **Tujuan yang diizinkan** — tools ini hanya boleh dipakai terhadap sistem
   ...
   dari pemiliknya untuk diuji (mis. scope kontrak pentest, lab pribadi, mesin
   CTF).

2. **Tanpa izin = ilegal** — mengakses, memindai, atau mengeksploitasi sistem
   komputer tanpa otorisasi melanggar hukum, termasuk tapi tidak terbatas pada
...
4. **Hasil scan bukan garansi** — temuan otomatis (OWASP scanner, auth bypass,
   dsb.) adalah **heuristik** dan bisa false-positive. Verifikasi manual wajib
   dilakukan sebelum ditindaklanjuti.

5. **Preview perintah** — aplikasi selalu menampilkan preview perintah sebelum
   eksekusi. Jangan pernah menyetujui eksekusi terhadap target di luar scope
   yang diizinkan.

**Use responsibly. Hak dan tanggung jawab ada di tanganmu.**
